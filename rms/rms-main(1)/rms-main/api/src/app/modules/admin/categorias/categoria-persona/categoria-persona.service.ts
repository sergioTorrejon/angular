...
export class CategoriaPersonaService {
  constructor(
    @InjectRepository(CategoriaPersona)
    private readonly repository: Repository<CategoriaPersona>) {
  }
...
//#region CRUD SERVICES
  ...
  async createOne(dto: CategoriaPersonaCreateDto) {
  ...
  async editOne(id: number, dto: CategoriaPersonaUpdateDto) {
    try{
...
/*   async getOptionsDetail() {
    const data = await  this.repository.findOne({ select:{categoria:true, codigo:true,nameSubCategoria:true}, where:{status:true}})
    return responseSuccess('Peticion Correcta',data);
  } */
